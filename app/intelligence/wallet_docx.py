...
class WalletDocxRenderer:
    ...
    def _wallet_profile(self, index: int, report: WalletIntelligenceReport) -> list[str]:
        rows: list[str] = [
            ...
            self._kv("Source", "Reconstructed investment positions, not isolated transactions"),
            ...
            self._heading("8A. Reconstructed Position Summary", 3),
            self._kv("Total Positions", report.position_summary.get("total_positions", "Insufficient historical data")),
            self._kv("Winning Positions", report.position_summary.get("winning_positions", "Insufficient historical data")),
            self._kv("Losing Positions", report.position_summary.get("losing_positions", "Insufficient historical data")),
            self._kv("Open Positions", report.position_summary.get("open_positions", "Insufficient historical data")),
            self._kv("Closed Positions", report.position_summary.get("closed_positions", "Insufficient historical data")),
            self._kv("Largest Winner", report.position_summary.get("largest_winner") or "Insufficient historical data"),
            self._kv("Largest Loser", report.position_summary.get("largest_loser") or "Insufficient historical data"),
            self._kv("Average Position Size", report.position_summary.get("average_position_size", "Insufficient historical data")),
            self._kv("Portfolio Diversification", report.position_summary.get("portfolio_diversification", "Insufficient historical data")),
            self._kv("Best Performing Token", report.position_summary.get("best_performing_token") or "Insufficient historical data"),
            self._kv("Worst Performing Token", report.position_summary.get("worst_performing_token") or "Insufficient historical data"),
            self._kv("Position Quality Distribution", report.position_summary.get("position_quality_distribution", {})),
            self._heading("8C. Market Context", 3),
            self._kv("Average Market Context Score", report.market_context_summary.get("average_context_score", "Insufficient Market Data")),
            self._kv("Positions With Context", report.market_context_summary.get("positions_with_context", 0)),
            self._kv("Positions Missing Context", report.market_context_summary.get("positions_missing_context", 0)),
            self._paragraph("; ".join(report.market_context_explanations[:3]) or "Insufficient Market Data"),
            self._heading("8B. Investment Timeline", 3),
            *(self._paragraph(item) for item in report.investment_timeline or ["Insufficient historical data"]),
